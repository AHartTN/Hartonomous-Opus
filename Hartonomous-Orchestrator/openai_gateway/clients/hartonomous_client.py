...
# On Windows, add DLL directory to PATH and preload dependencies
if platform.system() == "Windows" and _hypercube_dll_path:
    ...
    # Add to DLL search directories for Python 3.8+
    try:
    ...
    # Preload system dependencies first (PostgreSQL, OpenSSL)
    system_deps = ["libpq", "libcrypto-3-x64", "libssl-3-x64"]
    for dep_name in system_deps:
        dep_path = _find_dll(dep_name)
        if dep_path:
            try:
                ctypes.CDLL(str(dep_path))
                logger.info(f"Preloaded system dependency: {dep_name}.dll")
            except Exception as e:
                logger.warning(f"Could not preload {dep_name}.dll: {e}")

    # Preload core library dependencies (C bridge DLLs depend on these)
    core_deps = ["hypercube", "generative", "embedding_ops"]
    for dep_name in core_deps:
        dep_path = _find_dll(dep_name)
        if dep_path:
            try:
                ctypes.CDLL(str(dep_path))
                logger.info(f"Preloaded core dependency: {dep_name}.dll")
            except Exception as e:
                logger.warning(f"Could not preload {dep_name}.dll: {e}")

# Load C bridge DLLs (after dependencies are preloaded)
if _embedding_dll_path:
    try:
        _embedding_lib = ctypes.CDLL(str(_embedding_dll_path))
        logger.info("Loaded embedding_c.dll successfully")
    except Exception as e:
        _embedding_lib = None
        logger.error(f"Failed to load embedding_c.dll: {e}")
else:
    _embedding_lib = None
...
if _generative_dll_path:
    try:
        ...
        logger.info("Loaded generative_c.dll successfully")
    except Exception as e:
        _generative_lib = None
        logger.error(f"Failed to load generative_c.dll: {e}")
else:
    _generative_lib = None
...
if _hypercube_dll_path:
    try:
        ...
        logger.info("Loaded hypercube_c.dll successfully")
    except Exception as e:
        _hypercube_lib = None
        logger.error(f"Failed to load hypercube_c.dll: {e}")
else:
    _hypercube_lib = None
