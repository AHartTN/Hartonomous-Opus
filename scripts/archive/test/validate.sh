...
# Set PostgreSQL environment variables from HC_ config
...
ATOM_COUNT=$(psql -tAc "SELECT COUNT(*) FROM atom")
echo "  Atoms (codepoints): $ATOM_COUNT"
...
COMP_COUNT=$(psql -tAc "SELECT COUNT(*) FROM composition")
echo "  Compositions: $COMP_COUNT"

if [ "$COMP_COUNT" -gt 0 ]; then
    NULL_CENTROIDS=$(psql -tAc "SELECT COUNT(*) FROM composition WHERE centroid IS NULL")
    run_test "Compositions have centroids" "[ $NULL_CENTROIDS -eq 0 ]"

    BAD_DEPTH=$(psql -tAc "SELECT COUNT(*) FROM composition WHERE depth < 1")
    run_test "Compositions have depth >= 1" "[ $BAD_DEPTH -eq 0 ]"
else
...
run_test "atom_centroid()" "psql -tAc \"SELECT ST_X(atom_centroid((SELECT id FROM atom WHERE codepoint = 65))) IS NOT NULL\" | grep -q 't'"
...
echo "  ├─ Atoms:                $(psql -tAc "SELECT COUNT(*) FROM atom")"
echo "  ├─ Compositions:         $(psql -tAc "SELECT COUNT(*) FROM composition")"
echo "  ├─ Max composition depth: $(psql -tAc "SELECT COALESCE(MAX(depth), 0) FROM composition")"
echo "  └─ Database size:        $(psql -tAc "SELECT pg_size_pretty(pg_database_size(current_database()))")"
echo ""
