...
class OpusPostgresClient:
    ...
    def semantic_search_by_text(
        ...
        """
        Perform semantic search using SPATIAL OPERATIONS in 4D hypercube

        This is NOT flat vector cosine similarity!
        Semantics emerge from:
        1. 4D spatial distance (Euclidean in hypercube)
        2. Relations/edges with ELO ratings
        3. Graph connectivity patterns

        Pure database-native - no llama.cpp!

        Args:
            query_text: Query string (finds matching composition as query point)
            top_k: Number of results to return
            model_filter: Optional model name filter

        Returns:
            List of dicts with keys: id, model, layer, spatial_distance, metadata
        """
        self._ensure_connection()

        # Strategy: Find compositions matching query text, then find spatial neighbors
        query = """
            WITH query_comp AS (
                SELECT id, position_x, position_y, position_z, position_w
                FROM composition
                WHERE metadata->>'text' ILIKE %s
                LIMIT 1
            )
            SELECT
                ENCODE(c.id, 'hex') as id,
                c.model,
                c.layer,
                c.component,
                c.metadata,
                -- 4D Euclidean distance in hypercube space
                SQRT(
                    POWER(c.position_x - q.position_x, 2) +
                    POWER(c.position_y - q.position_y, 2) +
                    POWER(c.position_z - q.position_z, 2) +
                    POWER(c.position_w - q.position_w, 2)
                ) AS spatial_distance
            FROM composition c
            CROSS JOIN query_comp q
            WHERE c.metadata->>'text' IS NOT NULL
              AND c.id != q.id
        """
        params = [f'%{query_text}%']
        ...
        try:
            with self.conn.cursor() as cur:
                ...
                logger.info(f"Spatial search returned {len(results)} results (4D hypercube distance)")
                return [dict(row) for row in results]
            ...
            return self._simple_text_search(query_text, top_k, model_filter)
    ...
    def _simple_text_search(
        ...
        """Fallback: simple text matching without spatial operations"""
        query = """
            SELECT
                ENCODE(id, 'hex') as id,
                model,
