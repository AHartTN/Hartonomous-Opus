...
async def startup_event():
    ...
    For Hartonomous mode (USE_OPUS_DB=true), this loads the vocabulary,
    bigram, and attention caches from PostgreSQL - essentially "loading the model"
    but the model IS the database.
    """
    logger.info("Initializing OpenAI Gateway...")
    initialize_collections()

    # Initialize Hartonomous caches if using Opus database
    from .config import USE_OPUS_DB
    if USE_OPUS_DB:
        try:
            from .clients.hartonomous_loader import initialize_hartonomous_caches
            logger.info("Loading Hartonomous caches from database...")
            initialize_hartonomous_caches()
            logger.info("Hartonomous caches loaded successfully")
        except Exception as e:
            logger.error(f"Failed to load Hartonomous caches: {e}", exc_info=True)
            logger.warning("Continuing without Hartonomous caches - generation will not work")
