...
Projection4D project_embedding(const std::vector<float>& embedding, int seed) {
    ...
    const double coord_scale = 2147483647.0;  // Scale to int32 range
    ...
    // Scale to integer coordinates using tanh to bound
    Projection4D result;
    result.x = static_cast<int32_t>(std::tanh(proj[0]) * coord_scale);
    result.y = static_cast<int32_t>(std::tanh(proj[1]) * coord_scale);
    result.z = static_cast<int32_t>(std::tanh(proj[2]) * coord_scale);
    result.m = static_cast<int32_t>(std::tanh(proj[3]) * coord_scale);
